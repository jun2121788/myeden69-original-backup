...
if (!function_exists('get_user_menu')) {
    ...
    function get_user_menu() {
        ...
        $options[] = array(
            'name' => __('Change email', 'pop'),
            'url' => osc_change_user_email_url(),
            'class' => 'opt_change_email'
        );
        $options[] = array(
            'name' => __('Change username', 'pop'),
...
function pop_twitter_share_url() {
    if (osc_is_public_profile()) {
        $text = sprintf(__("Check out this profile with interesting products at %1\$s. %2$", 'pop'), osc_page_title(), osc_user_public_profile_url());
    }
    if (osc_is_ad_page()) {
        $text = sprintf(__("Check out what I just found at %1\$s. %2$", 'pop'), osc_page_title(), osc_item_url());
    }
...
//return '';
    ...
    echo '.custom-wrapper { background-color: #' . $p['mainColor'] . '}' .
        ...
        'body.item .user-detail .listing-seller:hover { border-bottom: 1px solid #'.$p['mainColor'].'; }',
